...
public class ISchemaUrl2PathDaoTest {
  ...
  /**
   * Test of findByMetadataId method, of class ISchemaUrl2PathDao.
   */
  @Test
  public void testFindByUrl() {
    System.out.println("testFindByUrl");
    String metadataId = "metadataId";
//    ISchemaUrl2PathDao instance = new ISchemaUrl2PathDaoImpl();
    Optional<SchemaUrl2Path> result;
    for (int i = 1; i < 7; i++) {
...
//    assertEquals(2, result.get().getNameCount());
  ...
  private void prepareDataBase() {
    ...
    for (String[] dataset : datasets) {
      saveSchemaUrl2Path(dataset[0], Paths.get(dataset[1], dataset[2]),dataset[2] + ".0.0");
    }
    assertEquals(datasets.length, dataRecordDao.count());
  }

  private void saveSchemaUrl2Path(String metadataId,
          Path metadataDocumentUri, String version) {

    SchemaUrl2Path dataRecord = new SchemaUrl2Path();
    dataRecord.setVersion(version);
    dataRecord.setUrl(metadataId);
    dataRecord.setSchemaId(metadataId);
    dataRecord.setPath(metadataDocumentUri.toString());
    dataRecord.setMimetype(MediaType.APPLICATION_JSON_VALUE);
