...
public class MetadataControllerFilterV2Test {
  ...
  public void registerSchemaDocument(String schemaType, String schemaId) throws Exception {
    ...
    MockMultipartFile schemaFile = switch (schemaType) {
      case "JSON" -> {
        record.getFormats().add(MediaType.APPLICATION_JSON_VALUE);
        yield new MockMultipartFile("schema", "schema.json", "application/json", JSON_SCHEMA.getBytes());
      }
      case "XML" -> {
        record.getFormats().add(MediaType.APPLICATION_XML_VALUE);
        yield new MockMultipartFile("schema", "schema.xsd", "application/xml", XML_SCHEMA.getBytes());
      }
      default -> throw new Exception("Unknown schema type!");
    };
  ...
  private void prepareEnvironment() {
    System.out.println("------MetadataControllerFilterV2Test----------------");
