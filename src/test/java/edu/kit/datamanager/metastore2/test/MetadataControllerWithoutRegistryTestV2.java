...
/**
 * Test for the MetadataController without a registry.
 * This test checks that the MetadataController works correctly without a registry.
 */
...
//    aclEntries.add(new AclEntry("test2",PERMISSION.ADMINISTRATE));
    ...
    MockMultipartFile metadataFile = new MockMultipartFile("document", "metadata.xml", "application/xml", CreateSchemaUtil.KIT_DOCUMENT.getBytes());
    // Works also without registry.
  ...
  public void testLandingpageSchema() throws Exception {
    this.mockMvc.perform(get("/api/v2/schemas/anything")
