...
/**
 * Test class for the MetadataController API v2.
 */
...
@TestPropertySource(properties = {"metastore.metadata.landingpage=/metadata-landing-page?id=$(id)&version=$(version)"})
@TestPropertySource(properties = {"metastore.schema.landingpage=/schema-landing-page?schemaId=$(id)&version=$(version)"})
@TestPropertySource(properties = {"repo.search.url=http://localhost:41421"})
@DirtiesContext(classMode = DirtiesContext.ClassMode.AFTER_CLASS)
public class MetadataControllerV2Test {
  ...
  public void setUp() throws Exception {
    System.out.println("------MetadataControllerV2Test API V2 ------------------");
...
//    record.setRelatedResource(RELATED_RESOURCE_2CE_2);
  ...
  public void testCreateRecordWithLocationUri() throws Exception {
    ...
    SchemaRegistryControllerV2Test.validateDataResources(data1, data2);
  }
...
//    Set<AclEntry> aclEntries = new HashSet<>();
  ...
  public void testFindRecordsOfMultipleVersionsBySchemaId() throws Exception {
    ...
    String schema1 = SchemaRegistryControllerV2Test.ingestOrUpdateXmlSchemaRecord(mockMvc, schemaId, XML_SCHEMA_V1, metadataConfig.getJwtSecret(), false, status().isCreated());
    String schema2 = SchemaRegistryControllerV2Test.ingestOrUpdateXmlSchemaRecord(mockMvc, schemaId, XML_SCHEMA_V2, metadataConfig.getJwtSecret(), true, status().isOk());
    String schema3 = SchemaRegistryControllerV2Test.ingestOrUpdateXmlSchemaRecord(mockMvc, schemaId, XML_SCHEMA_V3, metadataConfig.getJwtSecret(), true, status().isOk());
    ObjectMapper map = new ObjectMapper();
  ...
  public void testFindRecordsBySchemaIdANDResourceId() throws Exception {
    ...
    createDCMetadataRecordWithRelatedResource(relatedResource, firstSchemaId);
    createDCMetadataRecordWithRelatedResource(relatedResource2, firstSchemaId);
    createDCMetadataRecordWithRelatedResource(relatedResource, secondSchemaId);
    createDCMetadataRecordWithRelatedResource(relatedResource2, secondSchemaId);
...
//    Assert.assertEquals(record.getSchema().getIdentifier(), record2.getSchema().getIdentifier());
    SchemaRegistryControllerV2Test.validateRelatedIdentifierSets(record.getRelatedIdentifiers(), record2.getRelatedIdentifiers());
...
//    result = this.mockMvc.perform(put(API_METADATA_PATH + "dc").header("If-Match", etag).contentType(DataResourceRecordUtil.DATA_RESOURCE_MEDIA_TYPE).content(mapper.writeValueAsString(record))).andDo(print()).andExpect(status().isOk()).andReturn();
    ...
    DataResource record2 = mapper.readValue(body, DataResource.class);
//    Assert.assertNotEquals(record.getDocumentHash(), record2.getDocumentHash());
    SchemaRegistryControllerV2Test.validateCreateDates(oldRecord.getDates(), record2.getDates());
    ...
    SchemaRegistryControllerV2Test.validateSets(oldRecord.getAcls(), record2.getAcls());
...
//    Assert.assertEquals(dcMetadata, content);
  ...
  public void testDeleteSchemaWithLinkedDocument() throws Exception {
    ...
    this.mockMvc.perform(delete(API_SCHEMA_PATH + schemaId).
            ...
            andReturn();
    // Now it' gone
    this.mockMvc.perform(get(API_SCHEMA_PATH + schemaId).
            header("Accept", DataResourceRecordUtil.DATA_RESOURCE_MEDIA_TYPE)).
            andDo(print()).
            andExpect(status().isNotFound()).
            andReturn();
  ...
  private String createDCMetadataRecordWithRelatedResource(String myRelatedResource, String schemaId) throws Exception {
    ...
    DataResource record = SchemaRegistryControllerV2Test.createDataResource4JsonDocument(randomId, schemaId);
