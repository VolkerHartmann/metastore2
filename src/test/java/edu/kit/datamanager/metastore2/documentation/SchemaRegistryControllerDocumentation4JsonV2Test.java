...
@ActiveProfiles("test")
@TestPropertySource(properties = {"server.port=41427"})
...
@TestPropertySource(properties = {"metastore.schema.schemaFolder=file:///tmp/metastore2/v2/restdocu/json/schema"})
...
public class SchemaRegistryControllerDocumentation4JsonV2Test {
  ...
  private final static String TEMP_DIR_4_ALL = "/tmp/metastore2/v2/restdocu/json/";
  ...
  private final static String SCHEMA_V3 = """
              {
                ...
                "properties": {
                  ...
                  "note": {
                      "type": "string",
                      "title": "Note",
                      "description": "Additional information about object"
                  }
                },
                "additionalProperties": false
              }""";
  ...
  public void documentSchemaRegistry4Json() throws Exception {
    ...
    String endpointSchema = contextPath + "/api/v2/schemas/";
    String endpointMetadata = contextPath + "/api/v2/metadata/";
    ...
    String location = this.mockMvc.perform(MockMvcRequestBuilders.multipart(endpointSchema).
            ...
            andDo(document("v2-register-json-schema")).
            andExpect(status().isCreated()).
            andReturn().getResponse().getHeader("Location");
    String exampleSchemaV1 = location;
    ...
    String etag = this.mockMvc.perform(get(endpointSchema + "/" + EXAMPLE_SCHEMA_ID).
            ...
            andDo(document("v2-get-json-schema-record")).
    ...
    MvcResult result = this.mockMvc.perform(MockMvcRequestBuilders.multipart(endpointSchema + "/" + EXAMPLE_SCHEMA_ID).
            file(schemaFile).
            contextPath(contextPath).
            header("If-Match", etag).with(putMultipart())).
            andDo(document("v2-update-json-schema-v2")).
            andExpect(status().isOk()).
            andReturn();
    etag = result.getResponse().getHeader("ETag");
    String exampleSchemaV2 = result.getResponse().getHeader("location");
    ...
    result = this.mockMvc.perform(MockMvcRequestBuilders.multipart(endpointSchema + "/" + EXAMPLE_SCHEMA_ID).
            file(schemaFile).
            contextPath(contextPath).
            header("If-Match", etag).with(putMultipart())).
    ...
    location = this.mockMvc.perform(MockMvcRequestBuilders.multipart(endpointSchema).
            ...
            file(recordFile).
            contextPath(contextPath)).
            andDo(document("v2-register-another-json-schema")).
            andExpect(status().isCreated()).
            andReturn().getResponse().getHeader("Location");
    ...
    this.mockMvc.perform(get(endpointSchema).
            contextPath(contextPath)).
            andDo(document("v2-get-all-json-schemas")).
            andExpect(status().isOk()).
            andReturn().getResponse();
    ...
    this.mockMvc.perform(get(endpointSchema).
            ...
            contextPath(contextPath)).
            andDo(document("v2-get-all-json-schemas-pagination")).
            andExpect(status().isOk()).
            andReturn().getResponse();
    ...
    this.mockMvc.perform(get(endpointSchema).
            ...
            contextPath(contextPath)).
            andDo(document("v2-get-all-versions-of-a-json-schema")).
            andExpect(status().isOk()).
            andReturn().getResponse();
    ...
    this.mockMvc.perform(MockMvcRequestBuilders.multipart(endpointSchema + "/" + EXAMPLE_SCHEMA_ID + "/validate").
            file(metadataFile_v3).
            contextPath(contextPath)).
            andDo(document("v2-validate-json-document-v3")).
    ...
    result = this.mockMvc.perform(get(endpointSchema + "/" + EXAMPLE_SCHEMA_ID).
            contextPath(contextPath).
    ...
    this.mockMvc.perform(MockMvcRequestBuilders.multipart(endpointSchema + "/" + EXAMPLE_SCHEMA_ID).
            ...
            contextPath(contextPath).
            header("If-Match", etag).with(putMultipart())).
            andDo(document("v2-update-json-schema-record")).
            andExpect(status().isOk()).
            andReturn().getResponse();
    ...
    SchemaRegistryControllerV2Test.setTitle(metadataRecord, "Title of first JSON metadata document");
...
//    record.setId("my_id");
    ...
    this.mockMvc.perform(get(location).accept("application/json").
            contextPath(contextPath)).
            andDo(document("v2-get-json-metadata-document")).
            andExpect(status().isOk()).
            andReturn().getResponse();
    ...
    this.mockMvc.perform(get(location).accept(DataResourceRecordUtil.DATA_RESOURCE_MEDIA_TYPE).
            contextPath(contextPath)).
            andDo(document("v2-get-json-metadata-record")).
            andExpect(status().isOk()).
            andReturn().getResponse();
    ...
    result = this.mockMvc.perform(MockMvcRequestBuilders.multipart(location).
            ...
            header("If-Match", etag).with(putMultipart())).
            andDo(print()).
            andDo(document("v2-update-json-metadata-record-v2")).
            andExpect(status().isOk()).
            andReturn();
    etag = result.getResponse().getHeader("ETag");
    ...
    //**************************************************************************
    // update once more to the newest version of schema
    // Get Etag
    this.mockMvc.perform(get(location).
            contextPath(contextPath).
            accept(DataResourceRecordUtil.DATA_RESOURCE_MEDIA_TYPE)).
            andDo(document("v2-get-json-metadata-record-v2")).
            andExpect(status().isOk()).
            andReturn().getResponse();
    SchemaRegistryControllerV2Test.setRelatedSchema(record, exampleSchemaV3);
    ...
    result = this.mockMvc.perform(MockMvcRequestBuilders.multipart(newLocation).
            ...
            header("If-Match", etag).with(putMultipart())).
            andDo(print()).
            andDo(document("v2-update-json-metadata-record-v3")).
            andExpect(status().isOk()).
            andReturn();
    location = result.getResponse().getHeader("Location");
    Assert.assertNotNull(location);
    this.mockMvc.perform(get(location).
            contextPath(contextPath)).
            andDo(document("v2-get-json-metadata-document-v3")).
            andExpect(status().isOk()).
            andReturn().getResponse();
    ...
    this.mockMvc.perform(get(endpointMetadata).
            ...
            param("id", resourceId)).
            andDo(print()).
            andDo(document("v2-list-all-versions-of-json-metadata-document")).
            andExpect(status().isOk()).
            andReturn();
    ...
    this.mockMvc.perform(get(endpointMetadata).
            ...
            andDo(print()).
            andDo(document("v2-find-json-metadata-record-resource")).
            andExpect(status().isOk()).
            andReturn();
    ...
    this.mockMvc.perform(get(endpointMetadata).
            ...
            andDo(print()).
            andDo(document("v2-find-json-metadata-record-from")).
            andExpect(status().isOk()).
            andReturn();
    ...
    this.mockMvc.perform(get(endpointMetadata).
            ...
            andDo(print()).
            andDo(document("v2-find-json-metadata-record-from-to")).
            andExpect(status().isOk()).
            andReturn();
